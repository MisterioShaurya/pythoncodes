...
#voices = engine.getProperty('voices') # setting up
...
def guessthenumbergame():
    ...
    engine.say("Welcome to the Guess the Number game!")
    engine.runAndWait()

    while True:
        if i == 0:
            engine.say("Enter a number between 1 to 100: ")
            engine.runAndWait()
        ...
        if guess < randomn:
            ...
            engine.say("Higher number please.")
            engine.runAndWait()
        elif guess > randomn:
            print("Lower number please.")
            engine.say("Lower number please.")
            engine.runAndWait()
        else:
            print(f"WOW! You won the game in {i} guesses")
            engine.say(f"YAY! You won the game in {i} guesses")
            engine.runAndWait()
...
# Define responses for generic questions
generic_responses = [
...
def chatbot_response(message):
    message = message.lower()  # Convert input to lowercase
    if message in responses:
        ...
        guessthenumbergame()  # Call the number guessing game function
...
print("Jarvis:", time_greeting, get_random_response(greetings))  # Greet the user with appropriate time of the day
# Speak the text
engine.say(f"{time_greeting}{get_random_response(greetings)}")
